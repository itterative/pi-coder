...
describe("agent workspaces", () => {
    async function createClaimedWorkspace(repository: string, state: string) {
        ...
        const prepared = await updateAgentWorkspace(
            workspace,
            { setupState: "skipped" },
            { workspacesDir: state },
        );
    ...
    it("creates durable checkpoints without changing the parent checkout", async () => {
        ...
        await fs.writeFile(
            path.join(workspace.worktreePath, "untracked.txt"),
            "also checkpointed\n",
        );
        const beforeStatus = await gitOutput(
        ...
        expect(await gitOutput(repository, "rev-parse", checkpoint.durableRef)).toBe(
        ...
        expect(await gitOutput(workspace.worktreePath, "log", "-1", "--format=%s")).toBe(
            "pi-coder: workspace checkpoint",
        );
        expect(
            await gitOutput(
                workspace.worktreePath,
                "show",
                `${checkpoint.headRevision}:tracked.txt`,
            ),
        ).toBe("checkpointed");
        expect(
            await gitOutput(
                workspace.worktreePath,
                "show",
                `${checkpoint.headRevision}:untracked.txt`,
            ),
        ).toBe("also checkpointed");
        ...
        expect(
            await latestAgentWorkspaceCheckpoint(workspace.id, "worker-instance", {
                workspacesDir: state,
            }),
        ).toEqual(checkpoint);
        expect(await listAgentWorkspaceCheckpoints(workspace.id, { workspacesDir: state })).toEqual(
            [checkpoint],
        );
    });
    ...
    it("recycles an inactive checkpointed workspace while preserving history", async () => {
        ...
        expect(
            await gitOutput(
                recycled.worktreePath,
                "status",
                "--porcelain=v1",
                "--untracked-files=all",
            ),
        ).toBe("");
        expect(await getAgentWorkspaceCheckpoint(checkpoint.id, { workspacesDir: state })).toEqual(
            checkpoint,
        );
        ...
        await metadata.run(
            "UPDATE workspace_results SET reservation_token = ? WHERE id = ?",
            "held-by-other",
            result.id,
        );
        await metadata.close();
        await expect(
            applyAgentWorkspaceApplication(recycled, {
                ownerSessionId: "session-1",
                leaseRunId: "worker-1",
                leaseRunInstanceId: "worker-instance-1",
                resultId: result.id,
        ...
        await releasedMetadata.run(
            "UPDATE workspace_results SET reservation_token = NULL WHERE id = ?",
            result.id,
        );
        ...
        expect(await fs.readFile(path.join(repository, "tracked.txt"), "utf8")).toBe(
            "checkpointed\n",
        );
        expect(await fs.readFile(path.join(recycled.worktreePath, "new-worker.txt"), "utf8")).toBe(
            "new worker\n",
        );
        expect(await listAgentWorkspaceResults(leased.id, { workspacesDir: state })).toEqual([
            {
    ...
    it("restores a checkpoint into its original workspace", async () => {
        ...
        expect(await gitOutput(workspace.worktreePath, "rev-parse", "HEAD")).toBe(
            checkpoint.headRevision,
        );
        expect(await fs.readFile(path.join(workspace.worktreePath, "tracked.txt"), "utf8")).toBe(
            "first checkpoint\n",
        );
    ...
    it("creates flat random-slug worktrees and claims them atomically", async () => {
        ...
        expect(
            await findAvailableAgentWorkspace(repository, { workspacesDir: state }),
        ).toBeUndefined();
    ...
    it("releases an explicit stale clean task lease without resetting the worktree", async () => {
        ...
        const { workspace } = await createClaimedWorkspace(repository, state);
        const recovered = await releaseAgentWorkspaceLeaseForRecovery(workspace.id, {
            workspacesDir: state,
        });
        ...
        await expect(inspectAgentWorkspaceGitState(recovered)).resolves.toMatchObject({
            dirty: false,
        });
    });
    ...
    it("allows another session to reset or discard an inactive task lease", async () => {
        ...
        await fs.writeFile(
            path.join(resetTarget.workspace.worktreePath, "reset-me.txt"),
            "reset\n",
        );
        ...
        await expect(
            resetAgentWorkspaceForReuse(resetTarget.workspace.id, {
                ownerSessionId: "session-2",
                leaseRunId: "worker-1",
                workspacesDir: state,
            }),
        ...
        await completedDatabase.run(
            "UPDATE agent_runs SET status = 'completed' WHERE run_instance_id = ?",
            "session-1:worker-1",
        );
        ...
        await expect(
            fs.access(path.join(resetTarget.workspace.worktreePath, "reset-me.txt")),
        ).rejects.toThrow();

        const discardTarget = await createClaimedWorkspace(repository, state);
        await fs.writeFile(
            path.join(discardTarget.workspace.worktreePath, "discard-me.txt"),
            "discard\n",
        );
    ...
    it("does not recover a stale lease when the worktree is dirty", async () => {
        ...
        await expect(
            releaseAgentWorkspaceLeaseForRecovery(workspace.id, { workspacesDir: state }),
        ).rejects.toThrow("discard it explicitly");
        expect((await listAgentWorkspaces(repository, { workspacesDir: state }))[0]).toMatchObject({
            leaseRunId: "worker-1",
        });
    });
    ...
    it("limits each repository to the configured workspace capacity", async () => {
        ...
        await expect(createAgentWorkspace(repository, { workspacesDir: state })).rejects.toThrow(
        ...
        await expect(
            listAgentWorkspaces(repository, { workspacesDir: customState }),
        ).resolves.toMatchObject([{ id: subdirectoryWorkspace.id, cwd: subdirectory }]);
        await expect(
            createAgentWorkspace(repository, { workspacesDir: customState, maxWorkspaces: 1 }),
        ).rejects.toThrow("Workspace capacity reached");
    });
    ...
    it("allows manual creation from a dirty parent while using committed HEAD", async () => {
        ...
        expect(await fs.readFile(path.join(workspace.worktreePath, "README.md"), "utf8")).toBe(
            "committed\n",
        );
    ...
    it("releases a clean no-change result without creating a durable ref", async () => {
        ...
        await expect(
            findAvailableAgentWorkspace(repository, { workspacesDir: state }),
        ...
        expect(
            (await listAgentWorkspaceResults(workspace.id, { workspacesDir: state }))[0],
    ...
    it("does not reconcile an older no-change result from a newly claimed task lease", async () => {
        ...
        await expect(
            reconcileNoChangeAgentWorkspaceLeases(repository, { workspacesDir: state }),
        ).resolves.toBe(0);
        await expect(
            listAgentWorkspaces(repository, { workspacesDir: state }),
        ).resolves.toMatchObject([{ id: workspace.id, leaseRunId: "worker-2", leaseKind: "task" }]);
    });
    ...
    it("commits dirty tracked and untracked worker changes exactly once and persists the result", async () => {
        ...
        expect(await gitOutput(workspace.worktreePath, "log", "-1", "--format=%s")).toBe(
            "pi-coder: finalize isolated worker result",
        );
        expect(await inspectAgentWorkspaceGitState(workspace)).toMatchObject({
            dirty: false,
            changedFiles: 0,
        });
        expect(
            (await listAgentWorkspaces(repository, { workspacesDir: state }))[0]?.latestResult,
        ).toMatchObject({
            id: application.id,
        ...
        expect(
            await listAgentWorkspaceResults(workspace.id, { workspacesDir: state }),
        ).toHaveLength(1);
        expect(
            await gitOutput(
                workspace.worktreePath,
                "rev-parse",
                `refs/pi-coder/workspace-results/${workspace.id}/${application.id}`,
            ),
        ).toBe(application.workerHead);
    });
    ...
    it("does not create an empty final commit for an already-committed worker and applies without committing the parent", async () => {
        ...
        expect(await gitOutput(workspace.worktreePath, "log", "--format=%s", "-2")).not.toContain(
            "finalize isolated",
        );
        ...
        expect(
            (await listAgentWorkspaces(repository, { workspacesDir: state }))[0]?.leaseRunId,
        ).toBe("worker-1");
        await releaseAgentWorkspaceAfterApplication(workspace.id, {
            ownerSessionId: "session-1",
            leaseRunId: "worker-1",
    ...
    it("retains, inspects, resets, and discards changed workspace results explicitly", async () => {
        ...
        expect(await fs.readFile(path.join(repository, "parent-dirty.txt"), "utf8")).toBe(
            "keep parent changes\n",
        );
        const resetResult = (
            await listAgentWorkspaceResults(workspace.id, { workspacesDir: state })
    ...
    it("lets an owner discard a prepared result without requiring a clean parent", async () => {
        ...
        expect(
            await fs.readFile(path.join(workspace.worktreePath, "parent-advanced.txt"), "utf8"),
        ).toBe("parent advanced\n");
        expect(await gitOutput(workspace.worktreePath, "rev-parse", "HEAD")).toBe(parentHead);
        expect(await fs.readFile(path.join(repository, "parent-dirty.txt"), "utf8")).toBe(
            "leave me\n",
        );
    ...
    it("rejects dirty parents and applies clean descendant parent changes", async () => {
        ...
        await expect(
            releaseAgentWorkspaceAfterApplication(workspace.id, {
                ownerSessionId: "session-1",
                leaseRunId: "worker-1",
                workspacesDir: state,
            }),
        ...
        expect((await listAgentWorkspaces(repository, { workspacesDir: state }))[0]).toMatchObject({
            leaseRunId: "worker-1",
            latestResult: { status: "prepared", workerHead: application.workerHead },
        });
        ...
        expect(await fs.readFile(path.join(repository, "tracked.txt"), "utf8")).toBe("worker\n");
        expect(await fs.readFile(path.join(repository, "parent-advanced.txt"), "utf8")).toBe(
            "parent advanced\n",
        );
    });
    ...
    it("includes registered workspaces whose worktrees are missing when requested", async () => {
        ...
        await expect(
            listAgentWorkspaces(repository, {
                workspacesDir: state,
