...
- Scratch scripts: `npm run scratch -- <script> [args...]` bundles and runs one throwaway TS script from anywhere — `@pi-coder/...` imports repo code, `@earendil-works/...` is the external pi package, `./x` are sibling files, and any repo `node_modules` package bundles in; the child runs with `cwd` set to the script's directory. Details in `scripts/scratch.ts`.
- `tsconfig.json` covers only `src/`, so plain `npx tsc --noEmit` never sees test drift. `npm run typecheck:tests` (config `tsconfig.check.json`) typechecks `src` plus `test` under the same compiler options; use `npx tsc --noEmit --strict --target ES2022 --module ESNext --moduleResolution bundler --skipLibCheck --types node,vitest/globals <file>` for one focused test file. Both must stay at zero errors: they are what catches a fixture that no longer matches the contract it doubles.
- Scripted edits (heredoc python, `sed`) must assert their match count before writing and re-check the result
  afterwards: `str.replace` and `sed` both exit successfully when nothing matched, which has twice left dead
  code and a stale identifier in this repo. Prefer the `edit` tool for exact-string changes, and never write a
  partial batch — abort and re-read the file instead. See the `patch-hygiene` memory.
