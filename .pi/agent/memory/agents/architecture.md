...
- Built-ins: read-only `scout`, `reviewer`, and opt-in `advisor`, plus permission-gated `worker`; all built-ins include the `memories` and `scratchpad` capabilities. Foreground direct-user interaction is policy-controlled; the advisor is explicitly parent-guidance-only.
