...
- Every custom agent receives baseline `read`, `grep`, `find`, and `ls`. Optional capabilities include `memories`, `scratchpad`, `safe-bash`, and `command-runner`, while `edit` is reserved for the built-in worker. The `memories` capability loads pi-coder's memory extension in the child session; `scratchpad` creates a private temporary `/tmp` workspace in the child session. Legacy `tools` is ignored with a warning. A definition may specify `model`; otherwise it uses the parent model. A `todolist` capability is planned in `src/modules/todolist/PLAN.md`; its common frontmatter/schema parser, prompt, capability wiring, write/edit validation hooks, Bash change guard, parent widget, and volatile worker progress are implemented. Documentation and hardening remain.
