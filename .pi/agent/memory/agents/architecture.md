...
- Built-ins: read-only `scout`, `reviewer`, and opt-in `advisor`, plus permission-gated `worker`; all built-ins include the `memories` capability. Foreground direct-user interaction is policy-controlled; the advisor is explicitly parent-guidance-only.
...
- Every custom agent receives baseline `read`, `grep`, `find`, and `ls`. Optional capabilities include `memories`, `safe-bash`, and `command-runner`, while `edit` is reserved for the built-in worker. The `memories` capability loads pi-coder's memory extension in the child session. Legacy `tools` is ignored with a warning. A definition may specify `model`; otherwise it uses the parent model.
...
- Child sessions use `noExtensions: true` plus an explicit child extension, avoiding recursive pi-coder loading while retaining child-specific hooks such as confinement and `ask_parent`. Agents with the `memories` capability additionally receive the pi-coder memory extension.
