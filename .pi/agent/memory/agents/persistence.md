...
`/agents` scans active manager runs and persisted transcripts for the current cwd in one Agents view. Rows carry active or historical scope labels; details show user/custom/assistant messages and compact tool calls while omitting thinking, potentially large tool-result output, and hidden custom messages (`display: false`, e.g. pi-memory reminders and mailbox notes). Live details reload on agent events and follow new content until the user scrolls up; `End` resumes following. Details are read-only; `r` resumes interrupted active runs and `c` cancels waiting/interrupted runs. Historical and stale checkpoints remain read-only.
