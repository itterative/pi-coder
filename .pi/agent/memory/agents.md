...
- The `agent` tool supports foreground `start`, background `spawn`, `list`, `status`, `collect`, `resume`, `cancel`, isolated-result actions, and revision of collected non-mutating runs. Up to four runs may be active or interrupted; only one mutating worker may run at a time.
