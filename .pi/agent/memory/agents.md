...
- The `agent` tool supports `start` (foreground by default, or background with `background=true`), `list`, `status`, `collect`, `resume`, `cancel`, isolated-result actions, and revision of collected non-mutating runs. A running foreground `start` can be detached to the background with Ctrl+B; its start result returns collection guidance while the child continues. Up to four runs may be active or interrupted; only one mutating worker may run at a time.
