...
- Delegated runtime context uses bounded `context.sections` filtered and deduplicated by definition policies, with the complete rendered context block character-bounded and placed in the initial task message; start/spawn return a non-blocking warning to the parent when supplied sections are ignored by the target policy; automatic parent-summary/recent-context collection remains future work.
