...
- Read-only children are cwd-confined and `safe-bash` is heuristic `SAFE_READONLY`, not a security sandbox. Non-isolated workers inherit the parent permission state: same-checkout edits use existing access, outside-cwd file access uses the shared prompt, and unresolved Bash commands prompt with session-rule inheritance. Isolated workspaces keep independent mutation prompts and changed results outside the parent checkout until explicit disposition.
