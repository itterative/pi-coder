...
- **`spinner.ts`** — `Spinner`: small reusable animated frame component with configurable frames/interval, `setActive()`, and `dispose()`. It requests TUI renders while active and is used by the delegated-agent activity widget.
