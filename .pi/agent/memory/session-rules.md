...
- Session rules are per-registration parent-runtime state (not module globals), cleared on `session_start`, merged after config rules, and therefore win on identical patterns. They are never written to config or shown by `/bash-sandbox-config`.
