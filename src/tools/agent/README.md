...
Same-checkout worker edits inside the working directory use the parent's existing access and do not open a second mutation prompt. Outside-cwd reads and edits use the existing pi file-access prompt, including its session-scoped folder approval; sensitive paths and symlink escapes remain blocked. Worker bash honors configured and inherited session rules without prompting when already allowed. Unresolved commands use the parent-visible sandbox/direct prompt and can be remembered for the session. Isolated workers retain their independent mutation prompts and do not inherit parent session rules. Mutation calls are serialized through completion, so concurrent child tool calls cannot overlap mutations.
