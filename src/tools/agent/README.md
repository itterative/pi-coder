...
`spawn` returns immediately and lets up to four read-only children execute concurrently. A bounded above-editor activity widget shows running (`●`), waiting (`?`), ready (`✓`), and failed (`!`) runs along with each live run's current sanitized tool action plus a short preview of its latest assistant response; it also retains up to three recent ready/failed rows. It updates asynchronously as child progress arrives. Waiting and terminal background changes are also coalesced in a bounded parent mailbox. At the next real parent prompt, the mailbox queues one hidden custom message with `deliverAs: "nextTurn"`; it does not interrupt active work, wake an idle parent, or inject full child output. The marker contains only run IDs, statuses, and short question/result previews. Poll with `status`; once terminal, `collect` returns and consumes the retained result. A background child cannot open `ask_user`, because an unsolicited dialog could race the parent TUI; it pauses through `ask_parent` instead and can be resumed without becoming foreground. `status`, `resume`, `cancel`, and `collect` report only usage accrued since the previous parent tool result for that run.
...
6. After a background run settles, submit the next parent prompt and verify its hidden mailbox marker is available to the model without triggering a turn by itself; verify collecting/canceling before that prompt suppresses stale pending markers.
7. Have a background child call `ask_parent`; verify it waits without opening a direct-user dialog, its question arrives through the next-turn mailbox, then it resumes in the background and can be collected.
...
9. Reload while a child is waiting and while one is running; verify cleanup, mailbox clearing, and stale IDs without an orphaned request or dialog.
