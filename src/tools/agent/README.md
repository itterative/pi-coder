...
`spawn` returns immediately and lets up to four read-only children execute concurrently. A persistent footer status shows running (`●`), waiting (`?`), ready (`✓`), and failed (`!`) runs. A bounded below-editor activity widget shows each live run's current sanitized tool action plus a short preview of its latest assistant response; it also retains up to three recent ready/failed rows. Both update asynchronously as child progress arrives. Poll with `status`; once terminal, `collect` returns and consumes the retained result. A background child cannot open `ask_user`, because an unsolicited dialog could race the parent TUI; it pauses through `ask_parent` instead and can be resumed without becoming foreground. `status`, `resume`, `cancel`, and `collect` report only usage accrued since the previous parent tool result for that run.
