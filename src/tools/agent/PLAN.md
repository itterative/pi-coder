...
- On parent collection of a terminal isolated run, pi-coder finalizes the worker tree, records a separate workspace result, and attaches explicit guidance containing the workspace/result IDs and revisions. If the worker produced no changes, no durable ref is created, the lease is released, and the workspace becomes reusable immediately. If changes exist, the parent checkout is not changed and the parent is told to explicitly apply, retain, reset, or discard the result. No generic dependency setup code is required.
