...
- `src/tools/agent/index.ts` contains the registration function and composes the reorganized feature package. Top-level `lifecycle.ts`, `browser.ts`, and `action-dispatch.ts` own extension lifecycle state/hooks, `/agents` orchestration, and tool routing; child SDK concerns live under `child/`, the run state machine under `runs/`, definitions under `definitions/`, SQLite/catalog access under `storage/`, workspace behavior under `workspaces/`, and rendering/observability under `presentation/` and `observability/`. Source and tests import these canonical modules directly; temporary deep-import compatibility facades have been removed. `presentation/browser-models.ts` projects session and workspace state, Git summaries, notices, and allowed workspace actions before passing data to TUI components.
...
- optional bounded `status` snapshots and one-shot `collect`; spawn/resume outputs tell the parent not to poll or wait by sleeping, and to end its turn if no other work remains because automatic mailbox notifications arrive at waiting/terminal transitions;
...
- Workspace worktrees live at `.state/workspaces/<random-word-slug>/`; the slug is intentionally independent of the source path and task text. Workspace and agent metadata are stored in `.state/meta.sqlite` using the shared versioned migration helper in `src/common/sqlite.ts`; the normalized cwd helper remains useful for agent-session storage and workspace matching, but is not part of the workspace path. Each project is limited to three persistent workspaces; reaching the limit reports capacity instead of silently creating another one.
...
- If no ready workspace exists, pi-coder creates one and launches a limited internal setup worker before launching the requested task worker. The setup worker uses a dedicated setup definition with permission-gated bash, prepares dependencies and project-local environment artifacts inside the isolated worktree, and reports missing prerequisites rather than repairing them.
...
- Setup uses the same explicit mutation permission gate as the task worker, with non-interactive setup instructions and a default ten-minute timeout for bash commands. It still settles fully before the task worker starts so the setup report is complete.
...
- On parent collection of a terminal isolated run, pi-coder finalizes the worker tree, records a separate workspace result, and attaches explicit guidance containing the workspace/result IDs and revisions. If the worker produced no changes, no durable ref is created, the lease is released, and the workspace becomes reusable immediately. If changes exist, the parent checkout is not changed; the parent can inspect, apply, discard, or revise the result through agent actions, while retain/reset remain available in the TUI. No generic dependency setup code is required.
...
- The unified browser's workspace details view exposes direct inspect/apply/retain/reset/discard actions and an inline diff viewer. Parent agent actions provide the normal autonomous inspect/apply/discard/revise path; destructive TUI actions require in-overlay confirmation. Do not add a mandatory “mark reviewed” action; `review_required` is a passive state.
