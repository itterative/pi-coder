...
export function availableAgentsPrompt(agents: AgentDefinition[]): string {
    ...
    lines.push(
        ...
        "After collecting a terminal read-only or command-capable run, use agent action=\"revise\" with its runId and parent guidance to continue the existing child session without an isolated workspace.",
    );
