...
export function availableAgentsPrompt(agents: AgentDefinition[]): string {
    ...
    lines.push(
        ...
        "Scout, reviewer, advisor, and custom agents are read-only. Use advisor for explicit implementation guidance and tradeoff review. The built-in worker is the only mutation-capable child; same-checkout edits inherit the parent's access, while outside-cwd paths and unresolved bash commands use the parent-visible permission prompt. Isolated workers use their own mutation prompts. Same-checkout workers are single-flight, while workers in distinct isolated worktrees may run concurrently.",
