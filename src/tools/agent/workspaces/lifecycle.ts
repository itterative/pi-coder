...
async function requireNoResultReservation(
    ...
    if (reservation) {
        throw new Error(
            `Workspace ${workspaceId} has a result disposition in progress; try again shortly.`,
        );
    }
}
...
export async function recoverAgentWorkspaceLease(
    ...
    try {
        const workspace = await attachLatestWorkspaceResult(
            database,
        ...
        if (!oldOwnerSessionId)
            throw new Error(`Workspace ${workspaceId} has no recorded lease owner.`);
        ...
        if (updated.changes !== 1)
            throw new Error(`Workspace ${workspaceId} lease changed during recovery.`);
        const recovered = await attachLatestWorkspaceResult(
            database,
            await workspaceById(database, workspaceId),
        );
        if (!recovered)
            throw new Error(`Workspace ${workspaceId} disappeared during lease recovery.`);
        return recovered;
    } finally {
...
export async function releaseAgentWorkspaceLeaseForRecovery(
    ...
    try {
        ...
        if (workspace.leaseKind !== "task" || !workspace.leaseRunId || workspace.latestResult) {
            throw new Error(
                `Workspace ${workspaceId} does not have a recoverable stale task lease.`,
            );
        }
        const status = await git(workspace.worktreePath, [
            "status",
            "--porcelain=v1",
            "--untracked-files=all",
        ]);
        if (status) {
            throw new Error(
                `Workspace ${workspaceId} has uncommitted or untracked changes; discard it explicitly instead.`,
            );
        }
        const head = await git(workspace.worktreePath, ["rev-parse", "HEAD"]);
        if (head !== workspace.baseRevision) {
            throw new Error(
                `Workspace ${workspaceId} has committed changes beyond its base revision; discard it explicitly instead.`,
            );
        }
    ...
    try {
        return await database.transaction(async (database) => {
            ...
            if (await workspaceLeaseActive(database, workspace)) {
                throw new Error(
                    `Workspace ${workspaceId} is still used by active run ${previousLeaseRunId}; finish or cancel that run first.`,
                );
            }
        ...
        `,
            ...
            if (!recycled)
                throw new Error(
                    `Workspace ${workspaceId} disappeared while it was being recycled.`,
                );
            return recycled;
        }, "IMMEDIATE");
    } finally {
...
export async function resetAgentWorkspaceForReuse(
    workspaceId: string,
    { workspacesDir = PI_CODER_WORKSPACES_DIR }: AgentWorkspaceLeaseControls = {},
): Promise<AgentWorkspace> {
    const database = await openDatabase(workspacesDir);
    try {
        return await database.transaction(async (database) => {
            ...
            if (workspace.leaseRunId) {
                if (workspace.leaseKind !== "task") {
                    throw new Error(
                        `Workspace ${workspaceId} is leased for setup and cannot be reset.`,
                    );
                }
                if (await workspaceLeaseActive(database, workspace)) {
                    throw new Error(
                        `Workspace ${workspaceId} is still used by active run ${workspace.leaseRunId}; finish or cancel that run first.`,
                    );
                }
            ...
            await git(workspace.worktreePath, ["reset", "--hard", targetRevision]);
            await git(workspace.worktreePath, ["clean", "-fd"]);
            const refs = (await database.all(
                "SELECT durable_ref FROM workspace_results WHERE workspace_id = ? AND durable_ref IS NOT NULL",
                workspaceId,
            )) as WorkspaceRow[];
...
export async function discardAgentWorkspace(
    ...
    try {
        await database.transaction(async (database) => {
            ...
            if (workspace.status === "recycling") {
                throw new Error(
                    `Workspace ${workspaceId} is being recycled and cannot be discarded.`,
                );
            }
            if (workspace.leaseRunId) {
                if (workspace.leaseKind !== "task") {
                    throw new Error(
                        `Workspace ${workspaceId} is leased for setup and cannot be discarded.`,
                    );
                }
                if (await workspaceLeaseActive(database, workspace)) {
                    throw new Error(
...
export async function createAgentWorkspace(
    ...
    if (!skipParentDirtyCheck) {
        const parentStatus = await git(repositoryRoot, [
            "status",
            "--porcelain=v1",
            "--untracked-files=all",
        ]);
        if (parentStatus) {
            throw new Error(
                "Cannot create an isolated workspace while the parent checkout has uncommitted changes.",
            );
        }
    }
    ...
    try {
        return await database.transaction(async (database) => {
            ...
            const countRow = (await database.get(
                "SELECT COUNT(*) AS count FROM workspaces WHERE repository_root = ?",
                repositoryRoot,
