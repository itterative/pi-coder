...
function isCreateWorkspace(
    item: BrowserItem,
): item is Extract<WorkspaceListItem, { kind: "create" }> {
...
function tabText(
    ...
    const agents =
        tab === "agents" ? theme.fg("accent", theme.bold("● Agents")) : theme.fg("dim", "○ Agents");
    const tabs = [agents];
...
export class AgentSessionBrowserComponent
    implements Component, RefreshTarget<AgentSessionBrowserData>
{
    ...
    constructor(options: AgentSessionBrowserOptions) {
        ...
        this.refresh = options.onRefresh
            ? new RefreshCoordinator(options.onRefresh, this, (error) =>
    ...
    ): ListViewComponent<BrowserItem, void, BrowserState> {
        ...
        return new ListViewComponent<BrowserItem, void, BrowserState>(
            {
                ...
                headerContent: (container, theme) => {
                    this.tabHeader = new Text(
                        tabText(this.activeTab, theme, includeWorkspaces, includeSettings),
                        5,
                        0,
                    );
                ...
                renderItem: (
                    item: ListItem<BrowserItem>,
                    renderOptions: ListViewRenderItemOptions<BrowserItem, BrowserState>,
                ) => {
    ...
    private openSelected(
        selected: BrowserItem | undefined,
        options: AgentSessionBrowserOptions,
    ): void {
        ...
        const detail = new AgentSessionDetailComponent({
            item: selected,
            fixedHeight: options.fixedHeight,
        });
    ...
    private loadTranscriptForDetail(
        ...
        void this.onLoadTranscript(item)
            .then((updated) => {
    ...
    private openSetting(setting: AgentSetting, options: AgentSessionBrowserOptions): void {
        ...
        const modelItems = (
            this.models ?? [{ label: "Parent model", description: "Use the current pi model" }]
        ...
        const selector = new SelectComponent<AgentModelOption>({
            ...
            headerContent: (container, theme) => {
                container.addChild(
                    new Text(
                        theme.fg(
                            "muted",
                            "  Choose the AI model that should handle this agent's tasks.",
                        ),
                        1,
                        0,
                    ),
            ...
            initialCursor: Math.max(
                0,
                modelItems.findIndex((item) => item.value.id === setting.model),
            ),
            maxVisible: 12,
        ...
        selector.setDoneCallback((model) => {
            ...
            if (
                !current ||
                current.id === "notifyBusyWorkerChanges" ||
                current.id === "advisorEnabled"
            ) {
                this.invalidate();
                return;
            }
    ...
    private openMaxWorkspacesSetting(
    ...
    private closeListAndQueue(
        callback: AgentSessionBrowserOptions["onResume"] | AgentSessionBrowserOptions["onCancel"],
        item: AgentSessionBrowserItem,
    ): void {
        this.done?.();
    ...
    private switchTab(key: string, state: BrowserState): void {
        ...
        this.tabHeader?.setText(
            tabText(
                this.activeTab,
                this.theme!,
                this.tabs.includes("workspaces"),
                this.tabs.includes("settings"),
            ),
        );
        this.invalidate();
    }
    ...
    private updateWorkspace(
        workspace: AgentWorkspaceBrowserItem,
        replacement: AgentWorkspaceBrowserItem | null | undefined,
    ): void {
    ...
    private updateOpenDetails(): void {
        if (this.workspaceDetail) {
            const replacement = this.workspaces.find(
                (item) => item.id === this.workspaceDetail?.workspace.id,
            );
    ...
    private activeChild():
        | (
              ...
              | SelectComponent<AgentModelOption>
              | NumericInputComponent<number>
          )
        | null {
        return (
            this.sessionDetail ?? this.workspaceDetail ?? this.modelSelector ?? this.numericSelector
        );
    }
    ...
    private itemsForTab(tab: BrowserTab): ListItem<BrowserItem>[] {
        if (tab === "agents") {
            return asSessionListItems(this.agentItems(), this.loadingAgents).map(
                (item) => item as ListItem<BrowserItem>,
            );
        }
        if (tab === "workspaces") {
            return asWorkspaceListItems(
                this.workspaces,
                this.loadingWorkspaces,
                Boolean(this.onCreateWorkspace),
            ).map((item) => item as ListItem<BrowserItem>);
        }
    ...
    private rebuildItems(): void {
        ...
        this.list.state.cursor =
            selectedIndex >= 0
                ? selectedIndex
                : Math.min(
                      this.list.state.cursor ?? 0,
                      Math.max(0, this.list.state.items.length - 1),
                  );
        ...
        ctx.ui.custom<void>(
            (tui, theme, _keybindings, done) => {
                ...
                if (options.onInitialLoad) {
                    void options
                        .onInitialLoad(initialLoadController.signal)
                        .then((data) => {
                        ...
                        .catch((error: unknown) => {
                            ...
                            ctx.ui.notify(
                                `Could not load delegated-agent sessions: ${message}`,
                                "warning",
                            );
                        });
                }
