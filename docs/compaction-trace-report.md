...
| `estimate-skew`            | our estimate of a request and the tokens the provider counted for **that same request** (`usage.input + cacheRead + cacheWrite`) disagree by more than the band that estimate's own method can support: 15% when `estimateSource` is `usage-anchor` (+2% mean error measured) and 50% for whole-body chars/4 (+40% mean, +93% worst on the same session). It used to compare the estimate with pi's whole live-context count, a different body once the span truncates, so one healthy pipeline printed -34% and the next +21%. The detail names the method, the band and the direction: an estimate far above the count means the endpoint may have clipped the input (the silent-overflow shape pi's docs attribute to z.ai, MiMo, Ollama) or that the estimate over-reads the content; far below means the fit gate decided on a number too small |
